...
def get_existing_ids():
    ...
    for page in resp.get('results', []):
        t = page['properties']['VideoID']['title']
        if t:
            existing.add(t[0]['text']['content'])
    return existing
...
def translate_to_korean(text):
    if not text.strip():
        return "정보가 없습니다."
    prompt = (
        "다음 텍스트를 한국어로 번역하세요. 추가 정보 생성 금지. "
        "원본에 없는 내용은 넣지 말고, 빈 내용이면 '정보가 없습니다.'로 응답하세요.\n\n"
        + text
    )
...
def main():
    ...
    try:
        ...
        if e.resp.status == 403 and 'quotaExceeded' in str(e):
            print("❌ 쿼터가 초과되었습니다. 오늘은 더 이상 실행할 수 없습니다.")
            return
        else:
            raise
